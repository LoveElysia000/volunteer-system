...
| 模块 | 状态 | 说明 |
| --- | --- | --- |
| 阶段一-数据库 A（`volunteers` 扩展字段） | 🟡 | `total_hours` 已有；`total_points`、`level_id` 未落地 |
| 阶段一-数据库 B（`activities`） | ✅ | 活动表及相关 CRUD/报名流程已落地 |
| 阶段一-数据库 C（`activity_signups`） | ✅ | 报名表已落地，且已扩展审核/签到签退/工时字段 |
| 接口 1.1（`/api/v1/home/summary`） | ❌ | 未发现该接口实现 |
| 接口 1.2（活动列表基础版） | 🟡 | 列表已实现；`is_registered` 未在列表中按当前用户计算 |
| 接口 1.3（活动报名） | 🟡 | 报名接口已实现，但流程升级为“先审核后生效” |
| 阶段二-A（动态报名状态） | 🟡 | `is_full` 已有；`is_registered` 动态计算未完成 |
| 阶段二-B（报名校验） | ✅ | 名额校验与重复报名校验已实现（含待审核去重） |
| 阶段三（统计分析与等级） | ❌ | `records`、`level_rules`、月增统计、等级进度均未实现 |
| 阶段四-并发控制 | 🟡 | 有条件原子更新人数；完整高并发方案未完全按文档落地 |
| 阶段四-定时任务 | ❌ | 未发现活动自动归档定时任务 |
