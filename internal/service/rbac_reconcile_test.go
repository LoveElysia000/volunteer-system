...
func newRBACReconcileServiceForTest(t *testing.T) *RBACReconcileService {
	t.Helper()
...
func TestReconcileMissingDefaultRoles(t *testing.T) {
	svc := newRBACReconcileServiceForTest(t)

	reconciled, err := svc.ReconcileMissingDefaultRoles(10, 0)
	if err != nil {
	...
	err = svc.repo.DB.Table("rbac_account_roles ar").
		...
		Count(&count).Error
	if err != nil {
		t.Fatalf("count volunteer binding failed: %v", err)
	}
	if count != 1 {
		t.Fatalf("expected volunteer binding=1, got %d", count)
	}
	...
	err = svc.repo.DB.Table("rbac_account_roles ar").
		...
		Count(&count).Error
	if err != nil {
		t.Fatalf("count org binding failed: %v", err)
	}
	if count != 1 {
		t.Fatalf("expected org binding=1, got %d", count)
	}
}
